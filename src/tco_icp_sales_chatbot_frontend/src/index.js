...
//Note we will use "tco_icp_sales_chatbot_backend" in this JavaScript code a few times to call the backend
...
document.addEventListener('DOMContentLoaded', async (e) => {
  ...
  const question = await tco_icp_sales_chatbot_backend.getQuestion();
  ...
  const voteCounts = await tco_icp_sales_chatbot_backend.getVotes();
...
//via "await tco_icp_sales_chatbot_backend.vote(selectedOption)"
pollForm.addEventListener('submit', async (e) => {
  ...
  const updatedVoteCounts = await tco_icp_sales_chatbot_backend.vote(checkedValue);
...
resetButton.addEventListener('click', async (e) => {
    ...
    await tco_icp_sales_chatbot_backend.resetVotes();
    const voteCounts = await tco_icp_sales_chatbot_backend.getVotes();
    updateLocalVoteCounts(voteCounts);
